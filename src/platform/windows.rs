...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging MizeMoon.
...
pub fn try_kill_rustdesk_main_window_process() -> ResultType<()> {
    // Kill mizemoon.exe without extra arg, should only be called by --server
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\MizeMoon\MizeMoon.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\MizeMoon\MizeMoon.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
