...
hbb_name = 'mizemoon' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: mizemoon
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: mizemoon <info@mizemoon.ir>
Homepage: https://mizemoon.ir
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/mizemoon')
    system2('mkdir -p tmpdeb/etc/mizemoon/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/mizemoon/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/mizemoon || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/mizemoon/')
    system2(
        'cp ../res/mizemoon.service tmpdeb/usr/share/mizemoon/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/mizemoon.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/mizemoon.svg')
    system2(
        'cp ../res/mizemoon.desktop tmpdeb/usr/share/applications/mizemoon.desktop')
    system2(
        'cp ../res/mizemoon-link.desktop tmpdeb/usr/share/applications/mizemoon-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/mizemoon/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/mizemoon/')
    system2(
        'cp ../res/pam.d/mizemoon.debian tmpdeb/etc/pam.d/mizemoon')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/mizemoon/files/polkit && chmod a+x tmpdeb/usr/share/mizemoon/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb mizemoon.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('mizemoon.deb', '../mizemoon-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/mizemoon || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/mizemoon/')
    system2(
        'cp ../res/mizemoon.service tmpdeb/usr/share/mizemoon/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/mizemoon.png')
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/MizeMoon.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"MizeMoon Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon MizeMoon.app 200 190 --hide-extension MizeMoon.app mizemoon.dmg ./build/macos/Build/Products/Release/MizeMoon.app")
    os.rename("mizemoon.dmg", f"../mizemoon-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    if os.path.exists('./mizemoon_portable.exe'):
        os.replace('./target/release/rustdesk-portable-packer.exe',
                   './mizemoon_portable.exe')
    else:
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './mizemoon_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/mizemoon_portable.exe')
    os.rename('./mizemoon_portable.exe', f'./mizemoon-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/mizemoon-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/mizemoon.exe')
        system2('mv target/release/mizemoon.exe target/release/MizeMoon.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\mizemoon.exe')
        else:
            print('Not signed')
        system2(
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/mizemoon')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv mizemoon-%s-0-x86_64.pkg.tar.zst mizemoon-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./mizemoon.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/mizemoon-%s-0.x86_64.rpm ./mizemoon-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall mizemoon.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/mizemoon-%s-0.x86_64.rpm ./mizemoon-%s-suse.rpm' % (
                version, version))
        # yum localinstall mizemoon.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/mizemoon*.deb ./flutter/mizemoon.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/MizeMoon.app/Contents/MacOS/mizemoon')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/MizeMoon.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/mizemoon-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MizeMoon.app/Contents/MacOS/mizemoon
    #rcodesign sign --p12-file ~/.p12/mizemoon-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MizeMoon.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/mizemoon-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MizeMoon.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MizeMoon.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MizeMoon.app
    '''.format(pa))
                system2(
                    'create-dmg "MizeMoon %s.dmg" "target/release/bundle/osx/MizeMoon.app"' % version)
                os.rename('MizeMoon %s.dmg' %
                          version, 'mizemoon-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/mizemoon-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./mizemoon-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./mizemoon-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple mizemoon-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/MizeMoon.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/mizemoon*.deb ./mizemoon.deb')
                system2('dpkg-deb -R mizemoon.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/mizemoon/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/mizemoon-link.desktop tmpdeb/usr/share/applications/mizemoon-link.desktop')
                os.system('mkdir -p tmpdeb/etc/mizemoon/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/mizemoon/')
                os.system('mkdir -p tmpdeb/etc/X11/mizemoon/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/mizemoon/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/mizemoon.debian tmpdeb/etc/pam.d/mizemoon')
                system2('strip tmpdeb/usr/bin/mizemoon')
                system2('mkdir -p tmpdeb/usr/share/mizemoon')
                system2('mv tmpdeb/usr/bin/mizemoon tmpdeb/usr/share/mizemoon/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/mizemoon/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb mizemoon.deb; /bin/rm -rf tmpdeb/')
                os.rename('mizemoon.deb', 'mizemoon-%s.deb' % version)
